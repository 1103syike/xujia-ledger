 ...
 * 大版本寫成 XX.YY（兩段），不附敘述。
 */
export const CURRENT_APP_VERSION = '16.4.1';
...
/**
 * 版本歷程顯示順序（由上往下）：
 * 大版本線新→舊（16.4 → 16.3 → 16.2 → 16.1）；
 * 同線內大版本標題在上，小版由下往上遞增（16.4 → 16.4.1）。
 */
export const RELEASE_HISTORY: ReleaseEntry[] = [
  {
    version: '16.4',
    releasedAt: '2026-07-15T15:00:00+08:00',
    title: '',
    highlights: [],
  },
  {
    version: '16.4.1',
    releasedAt: '2026-07-16T09:00:00+08:00',
    title: '記帳建立大翻新：付款、服務費與總額同步',
    highlights: [
      '消費總額兩種模式：有設就固定分剩餘；沒設就淺色跟隨應付加總',
      '成員可「免均分」：只付自己的，不進共同（兩盒泡芙情境）',
      '輸入匡 focus／hover 動效統一（桃色光環＋微放大）',
      '誰在花錢可複選；誰在付錢改成頭像格多選，下方再填實付',
      '實付金額跟著分攤總和（含服務費）自動帶入',
      '服務費可選均分或依消費比例，並顯示在成員細項',
      '「全家」再點還原前狀態，成員 chip 有彈跳光澤',
      '每人多少補上平分共同消費、改秀分攤總和，不用分的人不顯示',
      '金額欄（總額、服務費、價格、實付等）只允許數字，擋中英文與科學記號',
      '修消費總額字串相加：應付加總不會再黏成 1231212880 這種怪數字',
      '全域按鈕按壓動效；金額欄點進去不再整段全選',
    ],
    sections: [
      {
        heading: '誰在付錢',
        items: [
          '改成頭像格多選，拿掉「多人付款」按鈕',
          '選中的人下方填實付金額（NT$ 一列）',
          '實付外殼單框，NT$ 與數字同一列，不再雙框重疊',
          '實付預設吃分攤總和（消費＋服務費），多人則均分尾差給第一人',
        ],
      },
      {
        heading: '消費總額',
        items: [
          '有設定總額（實色）：個人應付增減不會改掉總額；總額扣掉已填應付，其餘均分給沒鎖的人',
          '沒設定總額（淺色）：自動顯示應付加總，跟著個人應付變多變少',
          '樣式原則：自動分配→淺色；手動填寫→實色',
          '清掉總額欄位即可回到「跟隨應付」模式',
          '刪細項後總額可正常改／清，不會被舊分攤鎖死',
          '成員可點「免均分」：只付專屬細項，不進共同消費（例：自己一盒泡芙 300，其他人分另一盒）',
        ],
      },
      {
        heading: '服務費',
        items: [
          '可選「均分」或「照比例」（依基礎消費比例）',
          '服務費會出現在成員卡片細項，應付含服務費合計',
        ],
      },
      {
        heading: '誰在花錢與動效',
        items: [
          '成員 chip 可複選切換參與，至少保留一人；「全家」一次全選',
          '「全家」再點一次會還原成全選前的成員狀態',
          '點「全家」或成員名字 chip 會彈一下並掃過光澤；全家會讓成員依序接力彈',
          '金額送出後欄位／底部摘要有確認動效',
          '應付／實付為 0 時顯示空白，點進去不會自帶一個 0',
          '金額欄點擊只放游標，不再整段反藍全選',
          '全站按鈕／CTA／底部導覽統一按壓縮放回饋',
        ],
      },
      {
        heading: '交易明細',
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          '鍵盤開啟時提交列完全隱藏，不殘留陰影',
          '由下往上記細項時，專屬加總超過總額會自動拉高，單人付款實付預設為全部',
        ],
      },
    ],
  },
